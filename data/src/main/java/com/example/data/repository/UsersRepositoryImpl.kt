...
import com.example.data.model.UsersDto
...
class UsersRepositoryImpl @Inject constructor(
    ...
    suspend fun getUsers(): List<UsersDto> {
        try {
            ...
            val usersList = mutableListOf<UsersDto>()
            for (document in querySnapshot) {
                val user = document.toObject(UsersDto::class.java)
                if (user.role != "admin") {
                    usersList.add(user)
                }
            }
    ...
    suspend fun getUser(userId: String): UsersDto? {
        val userDocument = usersCollection.document(userId)
        val documentSnapshot = userDocument.get().await()
        return if (documentSnapshot.exists()) {
            documentSnapshot.toObject(UsersDto::class.java)
        } else {
            null
        }
    }
    ...
    suspend fun saveUser(user: UsersDto): String {
        return try {
